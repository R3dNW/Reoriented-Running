...
public class PlayerMovement : MonoBehaviour
{
    ...
    private Vector2 sensitivity = Vector2.one * 2;
    ...
    private float gripStrength = 20.0f;
    ...
    public void Start()
    {
        ...
        this.savedDrag = this.playerRigidbody.drag;
    ...
    public void GroundGrip()
    {
        ...
        if (Physics.Raycast(this.playerContainerTransform.position, -this.playerContainerTransform.up, out hitInfo, this.metalSurfaceMask))
        {
            if(hitInfo.distance <= 1.05)
            {
                this.grounded = true;
                this.playerRigidbody.drag = this.savedDrag;
            }
            else
            {
                this.grounded = false;
            ...
            {
                AttractToCollider(hitInfo.collider, 1.5f);
            }
            else
            {
                ...
                AttractToCollider(bestCollider, (1f / (bestSqrDist + 1)) * 1.5f);
            }
        }
    }
    ...
    public void AttractToCollider(Collider collider, float rotationRate)
    {
        ...
        this.playerContainerTransform.rotation = Quaternion.Slerp(
            ...
            Quaternion.LookRotation(
                Vector3.Cross(this.playerContainerTransform.right, -this.downDirection), -this.downDirection),
    ...
    public void Update()
    {
        ...
        {
            this.playerRigidbody.velocity += this.playerContainerTransform.up * this.jumpVelocity;
        }
